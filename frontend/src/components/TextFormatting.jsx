 ...
 * @param {string} text - Text yang akan di-render
 * @param {Object} options - Options untuk rendering (listType, spacing)
 * @returns {Array} Array of React elements
 */
export const renderStructuredText = (text, options = {}) => {
  if (!text) return ''
  
  const { listType = 'ul', spacing = 'normal' } = options
  const listClass = listType === 'ol' ? 'list-decimal' : 'list-disc'
  const ListElement = listType === 'ol' ? 'ol' : 'ul'
  const spacingClass = spacing === 'wide' ? 'mb-4' : 'mb-2'
  ...
  lines.forEach((line, index) => {
      ...
      if (currentList.length > 0) {
        elements.push(
          React.createElement(ListElement, {
            key: `list-${listKey++}`,
            className: `${spacingClass} ml-4 ${listClass}`
    ...
    // Check for empty line (untuk spacing)
    else if (trimmedLine === '') {
      // If we have a current list, render it first
      if (currentList.length > 0) {
        elements.push(
      ...
      // Add spacing element
      elements.push(
        <div key={`space-${index}`} className="mb-3"></div>
      )
    }
    // Regular line with inline formatting
    else if (trimmedLine) {
...
export const getFormatInstructions = (variant) => {
  const instructions = {
    ...
    structured: {
      ...
      format: '**Tajuk** untuk bold, - untuk bullet points, **bold** di mana-mana, kosongkan baris untuk spacing',
...
**Warranty**
- 2 tahun warranty penuh`,
      preview: 'Tajuk bold dengan ordered/unordered lists, inline formatting dan spacing'
    },
    whatsapp: {
